...
*/
       ...
       B[3][0]=1; B[3][1]=1;B[3][2]=1;B[3][3]=1;
        ...
        //naiveRecursiveMatrixMultiplication();
    }
   ...
   //*P3.6*//
    ...
    private static int[][] naiveRecursiveMatrixMultiplication(int size,int[][] A,int rowsA,int rowsAFinal,int columnsA,int columnsAFinal,
                                                              int B[][],int rowsB,int rowsBFinal,int columnsB,int columnsBFinal){
        if(rowsA==rowsAFinal){
            int[][] number= new int[1][1];
            number[0][0]=A[rowsA][columnsA]*B[rowsB][columnsB];
            return number;
        }else {
            int[][] C=new int[size/2][size/2];
            int[][] C11=sumMatrices(naiveRecursiveMatrixMultiplication(size/2,A,0,A.length/2-1,0,A[0].length/2-1,B,0,B.length/2-1,0,B[0].length/2-1)
                    ,naiveRecursiveMatrixMultiplication(A,0,A.length/2-1,A[0].length/2,A[0].length-1,B,B.length/2,B.length-1,0,B[0].length/2-1));

            int[][] C12=sumMatrices(naiveRecursiveMatrixMultiplication(A,0,A.length/2-1,0,A.length/2-1,B,0,B.length/2-1,B.length/2,B.length-1)
                    ,naiveRecursiveMatrixMultiplication(A,0,A.length/2-1,A.length/2,A.length-1,B,B.length/2,B.length-1,B.length/2,B.length-1));

            int[][] C21=naiveRecursiveMatrixMultiplication(A,A.length/2,A.length-1,0,A.length/2-1,B,0,B.length/2-1,0,B.length/2-1);
                        naiveRecursiveMatrixMultiplication(A,A.length/2,A.length-1,A.length/2,A.length/2-1,B,B.length/2,B.length-1,0,B.length/2-1);

            int[][] C22=naiveRecursiveMatrixMultiplication(A,A.length/2,A.length-1,0,A.length/2-1,B,0,B.length/2-1,B.length/2,B.length-1);
                        naiveRecursiveMatrixMultiplication(A,A.length/2-1,A.length-1,A.length/2-1,A.length-1,B,B.length/2,B.length-1,B.length/2,B.length-1);
